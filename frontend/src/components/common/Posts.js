...
const Posts = ({feedType, username, userId}) => {
	...
	return (
		<>
			...
			{!isLoading && !isRefetching && Array.isArray(posts)?.length === 0 && (<p className='text-center my-4'>No posts in this tab. Switch 👻</p>)}
			{!isLoading && !isRefetching && Array.isArray(posts) && (
			  ...
			  {!isLoading && !isRefetching && !Array.isArray(posts) && (
      			<p className="text-center my-4">Invalid posts data</p>
    )}
		</>
	);
};
